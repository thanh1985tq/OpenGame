...
export async function parseArguments(settings: Settings): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .command('$0 [query..]', 'Launch OpenGame CLI', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
...
export async function loadCliConfig(
  ...
  const nonInteractiveToolGating =
    !interactive &&
    !argv.experimentalAcp &&
    inputFormat !== InputFormat.STREAM_JSON;
  ...
  if (nonInteractiveToolGating) {
    if (approvalModeAutoElevated) {
      logger.warn(
        'Headless mode: defaulting approval mode to "auto-edit" so the model can write/edit files. ' +
      ...
      logger.warn(
        `Headless mode with approval mode "${
          approvalMode === ApprovalMode.PLAN ? 'plan' : 'default'
        }": write_file, replace, and run_shell_command are disabled, so the model cannot create or modify files. ` +
